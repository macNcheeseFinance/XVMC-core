...
interface IERC20 {
	...
	function swapOldForNew(uint256 amount) external returns (bool);
...
interface IMasterChef {
    ...
    function transferOwnership(address newOwner) external;
	function swapOldForNew(uint256 _amount) external;
}
...
contract XVMColdChefOwner {
    ...
    constructor(
        ...
        dummyToken.safeApprove(address(_masterchef), uint256(-1)); //to deposit&earn in old chef
		token.safeApprove(address(_newToken), uint256(-1)); //to exchange(swap) old for new token
    }
	...
	function harvestRewards() public {
		...
		newToken.swapOldForNew(token.balanceOf(address(this))); //swaps in new chef to get new token
	}
	...
	function burnTokens(uint256 _amount) public adminOnly {
		...
		if(_amount == 0) {
			newToken.burn(newToken.balanceOf(address(this)));
		} else {
			newToken.burn(_amount);
		}
	}
	...
	function renounceOwnership() external adminOnly notRenounced {
		dummyToken.updateOwner(); //makes sure the owner of dummy is updated to this address
		renounced = true;
	}
